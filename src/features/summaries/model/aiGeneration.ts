...
async function callEdgeFunction(
  ...
  const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate_weekly_summary`;
  return fetch(url, {
    method: "POST",
    headers: {
...
export async function generateWeeklySummary(
  ...
  try {
    ...
    // 4. Persist summary (only current week supported by repository for now)
    try {
      const persisted = await summariesRepository.createForIsoWeek({
      ...
      logDebug("Persistence error (non-fatal)", persistError);
      // Still treat as success so user sees generated content even if not saved
    }
