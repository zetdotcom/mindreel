...
export function useIsoWeekNavigation() {
  ...
  const getPreviousWeek = useCallback(
    (week: IsoWeekIdentifier): IsoWeekIdentifier => {
   ...
   */
  const parseWeekKeyToIdentifier = useCallback(
    (weekKey: WeekKey): IsoWeekIdentifier => {
  ...
  const areWeeksEqual = useCallback(
    (week1: IsoWeekIdentifier, week2: IsoWeekIdentifier): boolean => {
      return (
        week1.iso_year === week2.iso_year &&
        week1.week_of_year === week2.week_of_year
      );
    },
    [],
  );
  ...
  const compareWeeks = useCallback(
    (week1: IsoWeekIdentifier, week2: IsoWeekIdentifier): number => {
    ...
    (
      ...
      while (compareWeeks(currentWeek, endWeek) <= 0) {
        ...
        if (weeks.length > 105) {
          ...
          console.warn(
            "getWeeksBetween: Too many weeks, breaking to prevent infinite loop",
          );
          break;
        }
      }
