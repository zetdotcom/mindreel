...
import type { IsoWeekIdentifier, SummaryCardState } from "./types";
...
export function useSummaryOperations() {
  ...
  const generateSummary = useCallback(
    async (weekIdentifier: IsoWeekIdentifier): Promise<Summary | null> => {
  ...
  const updateSummary = useCallback(
    async (summaryId: number, content: string): Promise<Summary | null> => {
      try {
        setLoading(true);
        setError(null);
  ...
  const isGenerating = useCallback(
    (weekIdentifier: IsoWeekIdentifier): boolean => {
      const weekKey = `${weekIdentifier.iso_year}-W${weekIdentifier.week_of_year.toString().padStart(2, "0")}`;
