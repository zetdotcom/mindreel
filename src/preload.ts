...
const databaseApi = {
  // Entries
  createEntry: (input: CreateEntryInput) => ipcRenderer.invoke("db:createEntry", input),
  getEntryById: (id: number) => ipcRenderer.invoke("db:getEntryById", id),
  getEntries: (filters?: EntryFilters) => ipcRenderer.invoke("db:getEntries", filters),
  getTodayEntries: () => ipcRenderer.invoke("db:getTodayEntries"),
  getCurrentWeekEntries: () => ipcRenderer.invoke("db:getCurrentWeekEntries"),
  getEntriesForDate: (date: string) => ipcRenderer.invoke("db:getEntriesForDate", date),
  getEntriesForWeek: (weekOfYear: number) => ipcRenderer.invoke("db:getEntriesForWeek", weekOfYear),
  ...
  // Summaries
  createSummary: (input: CreateSummaryInput) => ipcRenderer.invoke("db:createSummary", input),
  ...
  getSummaryByWeek: (weekOfYear: number) => ipcRenderer.invoke("db:getSummaryByWeek", weekOfYear),
  ...
  getSummaryForDateRange: (startDate: string, endDate: string) =>
    ipcRenderer.invoke("db:getSummaryForDateRange", startDate, endDate),
  getCurrentWeekSummary: () => ipcRenderer.invoke("db:getCurrentWeekSummary"),
  getAllSummaries: () => ipcRenderer.invoke("db:getAllSummaries"),
  getSummariesByYear: (year: number) => ipcRenderer.invoke("db:getSummariesByYear", year),
  ...
  currentWeekSummaryExists: () => ipcRenderer.invoke("db:currentWeekSummaryExists"),
  ...
  summaryExistsForDateRange: (startDate: string, endDate: string) =>
    ipcRenderer.invoke("db:summaryExistsForDateRange", startDate, endDate),
  getLatestSummary: () => ipcRenderer.invoke("db:getLatestSummary"),

  // Settings
  getSettings: () => ipcRenderer.invoke("db:getSettings"),
  updateSettings: (input: UpdateSettingsInput) => ipcRenderer.invoke("db:updateSettings", input),
  updatePopupInterval: (minutes: number) => ipcRenderer.invoke("db:updatePopupInterval", minutes),
  ...
  getHistoryGroupingRules: () => ipcRenderer.invoke("db:getHistoryGroupingRules"),
  getHistoryGroupingSettings: () => ipcRenderer.invoke("db:getHistoryGroupingSettings"),
  updateHistoryGrouping: (input: UpdateHistoryGroupingInput) =>
    ipcRenderer.invoke("db:updateHistoryGrouping", input),
...
// Global shortcut API
const shortcutApi = {
  register: (accelerator: string | null) => ipcRenderer.invoke("shortcut:register", accelerator),
